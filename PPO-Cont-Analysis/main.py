...
def train(env_name, env_index, max_steps, device='cpu', write=True, render=False, 
          ...
          a_lr=3e-4, c_lr=3e-4, batch_size=64, random_steps=10000):
    ...
    while total_steps < max_steps:
        ...
        while not done and total_steps < max_steps:
            ...
            if total_steps < random_steps:
                ...
                agent.buffer.add(state, action, reward, 0, 0, terminated)
            else:
            ...
            if agent.buffer.count == agent.buffer.size:
                # Compute advantages before training
                if not done:
                    with torch.no_grad():
                        next_state_tensor = torch.FloatTensor(next_state).unsqueeze(0).to(device)
                        last_value = agent.critic(next_state_tensor).cpu().numpy()[0]
                else:
                    last_value = 0
